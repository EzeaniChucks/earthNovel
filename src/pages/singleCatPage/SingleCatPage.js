...
import { getTitleQuerySearch } from "../../features/book/bookSlice";
...
const SingleCatPage = () => {
  ...
  const handleSearchTitle = (e) => {
    ...
    dispatch(
      getTitleQuerySearch({
        genre: "all",
        queryString: `title=${e.target.value}`,
      })
    );
  };

  const handleTagClick = (e) => {
    //filter backend books based on tag clicked. Add active class to clicked tag
    dispatch(
      getFilteredBooks({
        genre: "all",
        queryString: `tag=${e.target.textContent}`,
      })
    );
    setPackedData({ ...packedData, clickedTag: e.target.textContent });
  };

  const elongateOrCollapseTags = () => {
    //display more or less taga
    collapseTags === 20
  ...
  useEffect(() => {
    //get book category on page load, using catName from params
    dispatch(getFilteredBooks({ genre: catName.toLowerCase() }));
  }, []);

  useEffect(() => {
    //get all tags available within each displayed book. Array is massaged to contain unique items
  ...
  useEffect(() => {
    //Bequeaths active class to specific tag after new search result is available
  ...
  return (
    <>
      ...
      {(error &&
        (filteredBooks === "timeout exceeded" ||
          filteredBooks === "Network Error")) ||
      ...
      {error &&
        (filteredBooks[0]?.title ||
          filteredBooks?.data?.msg === "No book in this genre yet") && (
          <div className="single-cat-container-error">
            <h1 className="h1-sorry">Oops!!!</h1>
            <h3 className="h3-sorry">
              There appears to be no books in this category yet
            </h3>
            <p>New books will be available soon</p>
            <p>
              Do well to check out other{" "}
              <Link
                to="/"
      ...
      {(filteredBooks?.length > 0 ||
        filteredBooks?.data?.msg === "Book doesn't exist") && (
        ...
        <div className="single-cat-container">
          ...
          <div className="tag-filter-container">
            ...
            <div className="cat-tag-display">
              ...
              {collatedTags.length > 20 && (
                <button onClick={elongateOrCollapseTags}>
            ...
            <div>
              ...
              <p>
                {filteredBooks?.length > 1
                  ...
                  : `${filteredBooks.length || "No"} story`}{" "}
                available in{" "}
                {packedData.clickedTag || packedData.pageTitle.toUpperCase()}{" "}
                category
              </p>
            </div>
            {!user && (
            ...
            <div className="blurb-display-container">
              ...
              {error && filteredBooks?.data?.msg === "Book doesn't exist" && (
                <div className="escape-grid">
                  ...
                  <h4>
                    Or simply click on the category/story tags{" "}
                    <span className="alternate">to the left</span>
                    <span className="alternate2">above</span>
                  </h4>
                </div>
              )}
              {/* search success */}
              {filteredBooks[0]?.title &&
                filteredBooks?.map((story) => {
                  return <SingleCatPerPage key={story._id} data={story} />;
                })}
            </div>
