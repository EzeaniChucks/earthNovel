...
export const getFilteredBooks = (packedData) => {
  if (packedData.queryString) {
    return API.get(
      `/search-books/${packedData.genre}?${packedData.queryString}`
    );
  }
  return API.get(`/search-books/${packedData.genre}`);
};
