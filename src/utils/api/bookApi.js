...
export const getFilteredBooks = (packedData) => API.get(`/search-books/${packedData.genre}?${packedData.queryString}`);
export const getSingleBook = (id) => API.get(`/single-book/${id}`);
export const editBook = (bookObj) => API.post(`/edit-book/${bookObj.id}`, bookObj.bookdata, { headers: { authorization: `Bearer ${token}` } });
...
export const picUpload = (fileObj) => {
    return API.post(`/upload/${fileObj.prevImageName}`, fileObj.imagedata, { headers: { authorization: `Bearer ${token}` } });
}